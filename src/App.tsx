...
import { PlayerData, InputState, Relic, BankGate, Game, Keeper, Trap } from './generated/types';
import { DebugPanel } from './components/DebugPanel';
import { GameScene } from './components/GameScene';
import { JoinGameDialog } from './components/JoinGameDialog';
import { HUD } from './components/HUD';
import { HowToPlay } from './components/HowToPlay';
import { KeeperVoice } from './components/KeeperVoice';
import { StatusOverlay } from './components/StatusOverlay';
import { PICKUP_RANGE, SHOVE_RANGE, GATE_RANGE } from './gameConstants';
...
function App() {
  ...
  // Dev-only HUD: hidden for the judges/demo build; append ?debug to the URL to show it.
  const showDebug = typeof window !== 'undefined' && new URLSearchParams(window.location.search).has('debug');

  // --- TOMB RUSH synced state ---
  const [relics, setRelics] = useState<ReadonlyMap<string, Relic>>(new Map());
  const [gates, setGates] = useState<ReadonlyMap<string, BankGate>>(new Map());
  const [traps, setTraps] = useState<ReadonlyMap<string, Trap>>(new Map());
  ...
  // Mirror gameplay state into refs for the proximity loop / action keys.
  const playersRef = useRef<ReadonlyMap<string, PlayerData>>(new Map());
  const relicsRef = useRef<ReadonlyMap<string, Relic>>(new Map());
  const gatesRef = useRef<ReadonlyMap<string, BankGate>>(new Map());
  const gameRef = useRef<Game | null>(null);
  const lastPickupRef = useRef<string | null>(null); // last relic id we tried to grab
  const lastBankRef = useRef<number>(0);             // last bank attempt timestamp
  ...
  const registerTableCallbacks = useCallback(() => {
    ...
    // --- TOMB RUSH tables ---
    const upsertRelic = (r: Relic) => setRelics(prev => new Map(prev).set(r.id.toString(), r));
    conn.db.relic.onInsert((_c: EventContext, r: Relic) => upsertRelic(r));
    conn.db.relic.onUpdate((_c: EventContext, _o: Relic, r: Relic) => upsertRelic(r));
    conn.db.relic.onDelete((_c: EventContext, r: Relic) =>
      setRelics(prev => { const m = new Map(prev); m.delete(r.id.toString()); return m; }));

    const upsertGate = (g: BankGate) => setGates(prev => new Map(prev).set(g.owner.toHexString(), g));
    conn.db.bank_gate.onInsert((_c: EventContext, g: BankGate) => upsertGate(g));
    conn.db.bank_gate.onUpdate((_c: EventContext, _o: BankGate, g: BankGate) => upsertGate(g));
    conn.db.bank_gate.onDelete((_c: EventContext, g: BankGate) =>
      setGates(prev => { const m = new Map(prev); m.delete(g.owner.toHexString()); return m; }));

    const upsertTrap = (t: Trap) => setTraps(prev => new Map(prev).set(t.id.toString(), t));
    conn.db.trap.onInsert((_c: EventContext, t: Trap) => upsertTrap(t));
    conn.db.trap.onUpdate((_c: EventContext, _o: Trap, t: Trap) => upsertTrap(t));
    conn.db.trap.onDelete((_c: EventContext, t: Trap) =>
      setTraps(prev => { const m = new Map(prev); m.delete(t.id.toString()); return m; }));

    conn.db.game.onInsert((_c: EventContext, g: Game) => setGame(g));
    conn.db.game.onUpdate((_c: EventContext, _o: Game, g: Game) => setGame(g));

    conn.db.keeper.onInsert((_c: EventContext, k: Keeper) => setKeeper(k));
    conn.db.keeper.onUpdate((_c: EventContext, _o: Keeper, k: Keeper) => setKeeper(k));
  ...
  const onSubscriptionApplied = useCallback(() => {
     ...
     // Backfill TOMB RUSH tables (v2 fires backfill immediately on subscribe).
     if (conn) {
        const rmap = new Map<string, Relic>();
        for (const r of conn.db.relic.iter()) rmap.set(r.id.toString(), r);
        setRelics(rmap);
        const gmap = new Map<string, BankGate>();
        for (const g of conn.db.bank_gate.iter()) gmap.set(g.owner.toHexString(), g);
        setGates(gmap);
        const tmap = new Map<string, Trap>();
        for (const t of conn.db.trap.iter()) tmap.set(t.id.toString(), t);
        setTraps(tmap);
        const gIt = [...conn.db.game.iter()];
        setGame(gIt.length ? gIt[0] : null);
        const kIt = [...conn.db.keeper.iter()];
        setKeeper(kIt.length ? kIt[0] : null);
     }
  }, []);
  ...
  // --- TOMB RUSH gameplay loop: auto-grab loose relics, auto-bank at your gate ---
  useEffect(() => {
    if (!connected) return;
    const handle = setInterval(() => {
      if (!conn || !identityRef.current) return;
      const me = localPlayerRef.current;
      const g = gameRef.current;
      if (!me || !g || g.phase !== 'playing') return;

      const carrying = me.carryingRelicId !== undefined && me.carryingRelicId !== null;
      if (!carrying) {
        let best: Relic | null = null;
        let bestD = (PICKUP_RANGE + 0.5) * (PICKUP_RANGE + 0.5);
        for (const r of relicsRef.current.values()) {
          if (r.state !== 'loose') continue;
          const dx = r.x - me.position.x;
          const dz = r.z - me.position.z;
          const d = dx * dx + dz * dz;
          if (d < bestD) { bestD = d; best = r; }
        }
        if (best) {
          const key = best.id.toString();
          // Only attempt each relic once (avoids a reducer storm on near-misses).
          if (lastPickupRef.current !== key) {
            conn.reducers.pickupRelic({ relicId: best.id });
            lastPickupRef.current = key;
          }
        }
      } else {
        lastPickupRef.current = null; // carrying now — reset so we can grab again later
        const gate = gatesRef.current.get(identityRef.current.toHexString());
        if (gate) {
          const dx = gate.x - me.position.x;
          const dz = gate.z - me.position.z;
          const now = performance.now();
          if (dx * dx + dz * dz <= (GATE_RANGE + 0.4) * (GATE_RANGE + 0.4) && now - lastBankRef.current > 700) {
            conn.reducers.bankRelic({});
            lastBankRef.current = now;
          }
        }
      }
    }, 220);
  ...
  // --- TOMB RUSH action keys: F = shove nearest robber, R = raid again, M = mute Keeper ---
  useEffect(() => {
    if (!connected) return;
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.code === 'KeyF') {
        const me = localPlayerRef.current;
        if (!me || !conn || !identityRef.current) return;
        const myHex = identityRef.current.toHexString();
        let best: PlayerData | null = null;
        let bestD = SHOVE_RANGE * SHOVE_RANGE;
        for (const p of playersRef.current.values()) {
          if (p.identity.toHexString() === myHex) continue;
          const dx = p.position.x - me.position.x;
          const dz = p.position.z - me.position.z;
          const d = dx * dx + dz * dz;
          if (d < bestD) { bestD = d; best = p; }
        }
        if (best) conn.reducers.shove({ target: best.identity });
      } else if (e.code === 'KeyR') {
        if (conn && gameRef.current?.phase === 'ended') conn.reducers.startRound({});
      } else if (e.code === 'KeyM') {
        setMuted(m => !m);
      }
    };
  ...
  useEffect(() => {
    ...
    // Configurable so the maincloud two-laptop demo needs no code edits:
    //   VITE_STDB_HOST=wss://maincloud.spacetimedb.com  VITE_STDB_DB=<your-db>
    const rawHost = (import.meta.env.VITE_STDB_HOST as string) || "localhost:3000";
    const dbUri = rawHost.startsWith("ws") ? rawHost : `ws://${rawHost}`;
    const dbName = (import.meta.env.VITE_STDB_DB as string) || "vibe-multiplayer";
  ...
  return (
    <div className="App" style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      {showHowToPlay && <HowToPlay onEnter={() => setShowHowToPlay(false)} />}
      {!showHowToPlay && showJoinDialog && <JoinGameDialog onJoin={handleJoinGame} />}
      ...
      {connected && (
        <>
          <GameScene
            players={players}
            relics={Array.from(relics.values())}
            gates={Array.from(gates.values())}
            traps={Array.from(traps.values())}
            nowTick={game ? Number(game.nowTick) : 0}
          ...
          <HUD
            players={players}
            game={game}
            localIdentityHex={identity?.toHexString() ?? null}
            onRestart={() => conn?.reducers.startRound({})}
          />
          <StatusOverlay
            player={localPlayer}
            nowTick={game ? Number(game.nowTick) : 0}
            gateSealedUntil={
              identity && gates.get(identity.toHexString())
                ? Number(gates.get(identity.toHexString())!.sealedUntilTick)
                : null
            }
          />
          <KeeperVoice keeper={keeper} muted={muted} />
        </>
      )}
